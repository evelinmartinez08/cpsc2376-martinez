...
void Sudoku::draw(SDL_Renderer* renderer, TTF_Font* font) const {
    drawGrid(renderer);
...
void Sudoku::drawNumbers(SDL_Renderer* renderer, TTF_Font* font) const {
    for (int i = 0; i < GRID_SIZE; ++i) {
        for (int j = 0; j < GRID_SIZE; ++j) {
            const Cell& cell = board[i][j];
            if (cell.value != 0) {
                SDL_Color color = (cell.state == CellState::FIXED) ? FIXED_COLOR : USER_COLOR;
                std::string text = std::to_string(cell.value);
                SDL_Surface* surface = TTF_RenderText_Solid(font, text.c_str(), color);
                SDL_Texture* texture = SDL_CreateTextureFromSurface(renderer, surface);

                SDL_Rect dst = { j * CELL_SIZE + 20, i * CELL_SIZE + 10, 20, 40 };
